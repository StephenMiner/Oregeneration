 ...
 */
    ...
    public void getWhereToReplace(Block origin, Material key, BlockState state) {
        new BukkitRunnable(){
            @Override
            public void run() {
                /*
               ...
               // replaceQueue.put(b.getLocation(), key);
                replenish(b.getLocation(), key, state);
    ...
    public void delayReplenishment(Block origin, Block replaceOn, Material mat, BlockState state){
        new BukkitRunnable() {
            @Override
            public void run() {
                replenish(replaceOn.getLocation(), mat, state);
    ...
    public void replenish(Location loc, Material mat, BlockState state){
        ...
        /*
        if (block.getState() instanceof Skull && state instanceof Skull){
            Skull old = (Skull) state;
            Skull skull = (Skull) block.getState();
            OfflinePlayer offline = old.getOwningPlayer();
            if (offline == null) System.out.println(111111);
            else{
    ...
    public void startReplenishment(Location loc, Material mat, BlockState state) {
        Block block = loc.getBlock();
        getWhereToReplace(block, fixMat(mat), state);
    }
